...
dict_list = []
while(1):
    print('len:', len(images_buffer), buffer_size)
    for i in range(buffer_size-len(images_buffer)):
        ...
        for j, vehicle in enumerate(data['vehicles']):
            ...
            # draw.text((x1, y1-50), vehicle['cls'] , font=fnt, fill=(0,255,0,20))
            ...
            if q1/q2 > 0.03: #0.03:
                ...
                if 'plate' in vehicle.keys():
                    ...
                    # print('pts:',pts)
                    for k in range(4):
                        pt1 = tuple(pts[:,k].astype(int).tolist())
                        pt2 = tuple(pts[:,(k+1)%4].astype(int).tolist())
                        # print(pt1,pt2)
                        draw.line([pt1, pt2],fill=(0,255,255), width= 8)
                        # cv2.line(I,pt1,pt2,color,thickness)

                    # cv2.imshow(str(j),vehicle['car']['image'])
                    # cv2.imshow(str(j),vehicle['plate']['image'])
                    # cv2.imshow('car',vehicle['plate']['car_image'])
                    # print(vehicle['plate']['image'].shape)
                    # # cv2.waitKey(500)

                if 'car' in vehicle.keys() and vehicle['car']['car_prob'] > 0.6:
