...
def reconstruct(Iorig,I,Y,out_size,threshold=.9):
	...
	if len(final_labels):
		...
		for i,label in enumerate(final_labels):
			...
			# print('t_ptsh :', t_ptsh)
			# print('Y :', Y.shape)
			# print('Affines:',Affines,Affines.shape, Iorig.shape)
			...
			# print('Iorig :',type(Iorig), Iorig.shape)


			TLps.append(Ilp)

	return final_labels,TLps, ptsh
...
def detect_lp(model,I,max_dim,net_step,out_size,threshold):
	...
	L,TLps,ptsh = reconstruct(I,Iresized,Yr,out_size,threshold)

	return L,TLps,ptsh, elapsed
